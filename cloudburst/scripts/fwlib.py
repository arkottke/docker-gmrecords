...
def process_fetches(
    mode: str, tmp_zip_dir: str, fetches: list, prior_errors: bool = False
):
    return_code = 0
...
def run_tasks(process_list: list, mode: str, prior_errors: bool = False):
    ...
    for process in process_list:
        ...
        rc = manage_process(
            proc_name, work_list, process["command"], log_behavior, exit_on_error
        )
        if exit_on_error and rc != 0:
...
def manage_process(
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=processor_count) as executor:
        for inputFilePath in work_list:
            ...
            futures.append(
                executor.submit(
                    process_runner, process_name, counter, len(work_list), cmd
                )
            )
...
def process_store(tasks_store: list, mode: str, prior_errors: bool = False):
    ...
    for task in tasks_store:
        ...
        compressSubDirectories = task.get("compressSubDirectories", False)
        remove_on_store = task.get("removeOnStore", False)
