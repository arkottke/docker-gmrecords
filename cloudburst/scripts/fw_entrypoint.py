    ...
    if cfg is None:
        ...
        for element in cfg:
            try:
                if element == "fetch":
                    fetches = cfg[element]
                    exit_code = fwlib.process_fetches(
                        mode_str, tmp_zip_dir, fetches, exit_code != 0
                    )
                    ...
                    exit_code = fwlib.run_tasks(
                        tasks_processing, mode_str, exit_code != 0
                    )
                    ...
                    if not local_mode:
                        ...
                        exit_code = fwlib.process_store(
                            tasks_store, mode_str, exit_code != 0
                        )
                    else:
                        exit_code = 0
